    ...
    try {
      ...
      if (analysis.missingMetaTitle > 0) {
        analysis.recommendations.push(
          `${analysis.missingMetaTitle} página(s) sin título meta`
        );
      }
      if (analysis.missingMetaDescription > 0) {
        analysis.recommendations.push(
          `${analysis.missingMetaDescription} página(s) sin descripción meta`
        );
      }
      if (analysis.missingKeywords > 0) {
        analysis.recommendations.push(
          `${analysis.missingKeywords} página(s) sin keywords`
        );
      }
      if (analysis.incompleteOpenGraph > 0) {
        analysis.recommendations.push(
          `${analysis.incompleteOpenGraph} página(s) con Open Graph incompleto`
        );
      }
      ...
      const maxPossibleIssues = analysis.totalPages * 5; // 5 criterios por página
