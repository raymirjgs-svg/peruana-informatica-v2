...
function ERPSyncWidget() {
  ...
  const fetchStatus = useCallback(async () => {
    try {
      const res = await fetch(`${API_CONFIG.BASE_URL}/api/sync/status`, { cache: 'no-store' });
  ...
  const forceSync = async () => {
    setForcing(true);
    try {
      const res = await fetch(`${API_CONFIG.BASE_URL}/api/sync/force`, { method: 'POST' });
      const json = await res.json();
      if (json.success) {
  ...
  const isActive = status?.isSyncing;
  const progressPct = status?.lastSyncTime && status?.nextSyncTime
    ? Math.min(100, Math.max(0, ((Date.now() - status.lastSyncTime) / status.intervalMs) * 100))
    : 0;
    ...
    <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-100 dark:border-gray-800 p-5">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2.5">
          <div className={`h-9 w-9 rounded-xl flex items-center justify-center shadow-md ${isActive ? 'bg-gradient-to-br from-blue-500 to-indigo-600 shadow-blue-500/20' : 'bg-gradient-to-br from-emerald-500 to-teal-600 shadow-emerald-500/20'}`}>
            <Zap className="h-4 w-4 text-white" />
          </div>
          <div>
            <h3 className="text-sm font-bold text-gray-900 dark:text-white">Sync ERP</h3>
            <p className="text-[11px] text-gray-400">Precios y stock</p>
          </div>
        </div>
        <div className={`flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[11px] font-semibold ${isActive ? 'bg-blue-50 dark:bg-blue-950/40 text-blue-600 dark:text-blue-400' : 'bg-emerald-50 dark:bg-emerald-950/40 text-emerald-600 dark:text-emerald-400'}`}>
          {isActive ? <RefreshCw className="h-3 w-3 animate-spin" /> : <CheckCircle2 className="h-3 w-3" />}
          {isActive ? 'Sincronizando…' : 'Listo'}
        </div>
      </div>

      <div className="grid grid-cols-2 gap-3 mb-4">
        <div className="bg-gray-50 dark:bg-gray-800/50 rounded-xl p-3">
          <p className="text-[10px] uppercase tracking-wider text-gray-400 mb-1">Último sync</p>
          <p className="text-xs font-semibold text-gray-700 dark:text-gray-300">
            {status ? formatRelative(status.lastSyncTime) : '—'}
          </p>
        </div>
        <div className="bg-gray-50 dark:bg-gray-800/50 rounded-xl p-3">
          <p className="text-[10px] uppercase tracking-wider text-gray-400 mb-1">Próximo sync</p>
          <p className={`text-xs font-bold tabular-nums ${status?.nextSyncTime && Date.now() > status.nextSyncTime - 120000 ? 'text-amber-500' : 'text-gray-700 dark:text-gray-300'}`}>
            {status ? formatCountdown(status.nextSyncTime) : '—'}
          </p>
        </div>
      </div>
      ...
      <button
        onClick={forceSync}
        disabled={forcing || isActive}
        className="w-full flex items-center justify-center gap-2 py-2 px-4 rounded-xl text-xs font-semibold transition-all duration-200 border border-gray-200 dark:border-gray-700 text-gray-600 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-800 disabled:opacity-50 disabled:cursor-not-allowed"
      >
        <RefreshCw className={`h-3.5 w-3.5 ${forcing ? 'animate-spin' : ''}`} />
        {forcing ? 'Iniciando…' : 'Sincronizar ahora'}
      </button>
    </div>
  );
}
